<?php
/**
 * File: roles/sarpras/verifikasi_ruangan.php
 */

check_role(['sarpras']);

// Proses Aksi
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['aksi_sarpras'])) {
    $id_peminjaman = (int)$_POST['id_peminjaman'];
    $aksi = $_POST['aksi_sarpras'];
    $catatan = sanitize_input($conn, $_POST['catatan_sarpras'] ?? '');

    $new_status = ($aksi === 'setuju') ? 'Disetujui' : 'Ditolak';

    $stmt = $conn->prepare("UPDATE peminjaman_tempat SET status_sarpras = ?, catatan_sarpras = ? WHERE id_peminjaman = ?");
    $stmt->bind_param("ssi", $new_status, $catatan, $id_peminjaman);
    
    if ($stmt->execute()) {
        $msg = "Persetujuan sarpras berhasil diproses!";
    } else {
...
<div class="container-fluid px-4 py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 fw-bold">Persetujuan Peminjaman Ruangan</h1>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-4">
                                ...
                                <div class="text-muted extra-small">ID: #R-<?php echo $row['id_peminjaman']; ?></div>
                            </td>
                            ...
                            <td>
                                <span class="badge bg-primary-subtle text-primary py-2 px-3 rounded-pill">
                                    <i class="bi bi-geo-alt me-1"></i> <?php echo htmlspecialchars($row['nama_ruangan']); ?>
                                </span>
                            </td>
                            ...
                            <td class="pe-4 text-center">
                                <?php if($row['status_sarpras'] == 'Pending'): ?>
                                    <button class="btn btn-primary btn-sm rounded-pill px-3 shadow-sm" data-bs-toggle="modal" data-bs-target="#modal<?php echo $row['id_peminjaman']; ?>">
                        ...
                        <!-- Modal -->
                        <div class="modal fade" id="modal<?php echo $row['id_peminjaman']; ?>" tabindex="-1">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <form action="" method="POST">
                                        <div class="modal-header">
                                            <h5 class="modal-title">Konfirmasi Persetujuan Sarpras</h5>
                                        ...
                                        <div class="modal-footer">
                                            <button type="submit" name="aksi_sarpras" value="tolak" class="btn btn-danger">Tolak</button>
                                            <button type="submit" name="aksi_sarpras" value="setuju" class="btn btn-success">Setujui</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                        </div>
                        <?php endwhile; if($res->num_rows == 0) echo "<tr><td colspan='6' class='text-center py-4'>Tidak ada pengajuan yang membutuhkan persetujuan sarpras.</td></tr>"; ?>
                    </tbody>
                </table>
            </div>
        </div>
